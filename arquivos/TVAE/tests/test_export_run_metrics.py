...
from tools import export_run_metrics
...
def test_collect_metric_sections_with_fallback_paths(tmp_path: Path) -> None:
    ...
    _write_json(
        baseline_dir / "metrics" / "metrics_tvae_order_1_hold.json",
        {"eval_split": "hold", "od_jsd": 0.01, "n_real": 10.0},
    )
    ...
    assert bundle["pipeline_context"]["baseline_run_dir"] == str(baseline_dir)
    ...
    assert sections["baseline_hold"]["status"] == "ok"
...
def test_export_outputs_json_and_csv(tmp_path: Path) -> None:
    ...
    assert ("baseline_hold", "od_jsd") in keys
