...
def _build_structure_metrics() -> list[tuple[str, str, str, bool, str]]:
    ...
    base_metrics: list[tuple[str, str, str, bool, str]] = [
        ...
        (
            ...
            "OD coverage (real)",
            False,
            "Real OD support coverage (higher is better).",
        ),
        (
            "od_coverage_synth",
            "od",
            "OD coverage (synth)",
            False,
            "Synthetic OD support coverage (higher is better).",
        ),
        (
            "od_unique_real",
            "od",
    ...
    base_metrics.extend(
        [
            (
                f"{passenger_col}_jsd",
                "passenger",
                f"{passenger_col} JSD",
                True,
                f"Distribution mismatch for {passenger_col} between real and synthetic.",
            ),
            (
                f"{passenger_col}_chi2",
                "passenger",
                f"{passenger_col} Chi²",
                True,
                f"Chi-square mismatch for {passenger_col} counts.",
            ),
            (
                f"{fare_col}_log1p_w1",
                "fare",
                f"{fare_col} log1p Wasserstein-1",
                True,
                f"Wasserstein-1 distance for log1p({fare_col}) distribution.",
            ),
            (
                f"{fare_col}_log1p_ks",
                "fare",
                f"{fare_col} log1p KS",
                True,
                f"KS statistic for log1p({fare_col}) distribution.",
            ),
            (
                f"{fare_col}_log1p_quantile_mae",
                "fare",
                f"{fare_col} log1p quantile MAE",
                True,
                f"Log1p quantile MAE for {fare_col} distribution.",
            ),
            (
                f"{fare_col}_log1p_median_mae_by_hour",
                "fare",
                f"{fare_col} log1p median MAE by hour",
                True,
                f"Hourly median profile MAE for log1p({fare_col}).",
            ),
            (
                f"{fare_col}_log1p_median_mae_by_{passenger_col}",
                "fare",
                f"{fare_col} log1p median MAE by {passenger_col}",
                True,
                f"Profile MAE for log1p({fare_col}) by {passenger_col} bins.",
            ),
            (
                "exact_match_rate_discrete",
                "residual",
                "Exact match rate (discrete)",
                False,
                "Exact-match privacy rate in discrete OD/time space.",
            ),
            (
                "exact_match_rate_with_r_rounded",
                "residual",
                f"Exact match rate with {residual_col} rounded",
                False,
                f"Exact-match privacy rate after residual ({residual_col}) rounding.",
            ),
        ]
    )
...
def _build_metric_aliases() -> dict[str, tuple[str, ...]]:
    passenger_col = str(getattr(config, "PASSENGER_COL", "passenger_count"))
    fare_col = str(getattr(config, "FARE_COL", "total_amount"))
    residual_col = str(getattr(config, "DISTANCE_RESIDUAL_COL", "r"))
    ...
    aliases: dict[str, tuple[str, ...]] = {
        ...
        "exact_match_rate_with_r_rounded": (
            "exact_match_rate_with_r_rounded",
            f"exact_match_rate_with_{residual_col}_rounded",
        ),
    }

    for key in (f"{passenger_col}_jsd", f"{passenger_col}_chi2"):
        aliases[key] = (
            key,
            "passenger_count_jsd" if key.endswith("_jsd") else "passenger_count_chi2",
            f"{passenger_col}_jsd" if key.endswith("_jsd") else f"{passenger_col}_chi2",
            "passenger_jsd" if key.endswith("_jsd") else "passenger_chi2",
        )
    ...
    for metric_suffix in (
        ...
        "log1p_ks",
        "log1p_quantile_mae",
        "log1p_median_mae_by_hour",
        f"log1p_median_mae_by_{passenger_col}",
    ):
        key = f"{fare_col}_{metric_suffix}"
        aliases[key] = (
            key,
            key.replace(fare_col, "fare"),
            key.replace(fare_col, "total_amount"),
            key.replace(fare_col, "fare_amount"),
        )

    return aliases


STRUCTURE_METRICS = _build_structure_metrics()
KEY_ALIASES = _build_metric_aliases()


def _metric_family_specs_from_sections(
    metrics_sections: Mapping[str, Mapping[str, Any]]
) -> list[tuple[str, str, str, bool, str]]:
    specs = list(STRUCTURE_METRICS)
    existing_keys = {metric_key for metric_key, *_ in specs}

    discovered: list[tuple[str, str, str, bool, str]] = []

    section_metric_keys: set[str] = set()
    for section in metrics_sections.values():
        if not isinstance(section, Mapping):
            continue
        section_metrics = section.get("metrics")
        if not isinstance(section_metrics, Mapping):
            continue
        for key in section_metrics.keys():
            section_metric_keys.add(str(key))

    for metric_key in sorted(section_metric_keys):
        if metric_key in existing_keys:
            continue
        if metric_key.startswith(("cov_", "dcr_", "rdcr_")) and metric_key.endswith("_ext"):
            label = f"{metric_key.replace('_', ' ').replace('ext', 'extended')}"
            discovered.append(
                (
                    metric_key,
                    "residual",
                    label,
                    metric_key.startswith("cov_") is False,
                    f"Residual-aware extended metric from {metric_key}.",
                )
            )
        elif metric_key in {"exact_match_rate_discrete", "exact_match_rate_with_r_rounded"}:
            discovered.append(
                (
                    metric_key,
                    "residual",
                    ...
                    False,
                    "Residual-aware privacy match-rate metric.",
                )
            )
...
def _collect_metric_comparison_rows(
    metrics_sections: Mapping[str, Mapping[str, Any]],
    metric_specs: list[tuple[str, str, str, bool, str]] | None = None,
) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    metric_rows: list[dict[str, Any]] = []
    summary_rows: list[dict[str, Any]] = []

    if metric_specs is None:
        metric_specs = _metric_family_specs_from_sections(metrics_sections)

    for metric_key, family, metric_label, lower_is_better, description in metric_specs:
...
def _build_tooltips(
    metric_specs: list[tuple[str, str, str, bool, str]]
    ...
    tooltips = {
        ...
        for metric_key, family, metric_name, lower_is_better, description in metric_specs
    }
...
def _write_tooltips_csv(
    path: Path, metric_specs: list[tuple[str, str, str, bool, str]]
) -> dict[str, dict[str, str]]:
    tooltips = _build_tooltips(metric_specs)
    fieldnames = [
        "metric_id",
        "metric_name",
        "metric_family",
...
def analyze_run_exports(
    *,
    ...
    if analyze_metrics and metrics_bundle is not None:
        metric_specs = _metric_family_specs_from_sections(metrics_sections)
        metrics_rows, metric_summary_rows = _collect_metric_comparison_rows(
            metrics_sections,
    ...
    if not metric_specs:
        metric_specs = _metric_family_specs_from_sections(metrics_sections)
    tooltips = _build_tooltips(metric_specs)
    _write_tooltips_csv(tooltips_csv, metric_specs)
