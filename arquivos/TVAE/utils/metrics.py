...
def od_counts(
    ...
    *,
    ...
    """Count OD pairs without temporal dimensions (pickup to dropoff)."""
    if df.empty:
...
def time_counts(
    ...
    *,
    ...
    """Count day-of-week and hour-of-day combinations."""
    if df.empty:
        return pd.Series(dtype="int64")
...
def joint_counts(
    ...
    *,
    ...
    time_cols: Tuple[str, str] = ("day_of_week", "hour_of_day"),
) -> pd.Series:
    if df.empty:
        return pd.Series(dtype="int64")
...
def od_metrics(
    ...
    *,
    ...
    """Metrics for OD marginal distribution (pickup × dropoff)."""
...
def time_metrics(
    ...
    *,
    ...
    """Metrics for temporal marginals (day-of-week × hour-of-day)."""
...
def joint_metrics(
    ...
    *,
    ...
    """Metrics for full joint distribution (OD × day × hour)."""
    ...
    # Mode dropping: real modes never generated by synthetic samples.
    missing = real_keys - synth_keys
    mode_dropping_ratio = len(missing) / max(1, len(real_keys))

    # Invalid modes: synthetic generated combinations that do not exist in real.
...
def time_key_series(
    df: pd.DataFrame, *, day_col: str = "day_of_week", hour_col: str = "hour_of_day"
) -> np.ndarray:
...
def distance_matrix(
    ...
    *,
    ...
    a_day = a_df["day_of_week"].to_numpy(dtype=np.int64)
    a_hour = a_df["hour_of_day"].to_numpy(dtype=np.int64)
    ...
    b_day = b_df["day_of_week"].to_numpy(dtype=np.int64)
    b_hour = b_df["hour_of_day"].to_numpy(dtype=np.int64)
...
def coverage_score(
    ...
    *,
    ...
    ref_day = ref_df["day_of_week"].to_numpy(dtype=np.int64)
    ref_hour = ref_df["hour_of_day"].to_numpy(dtype=np.int64)
    ...
    other_day = other_df["day_of_week"].to_numpy(dtype=np.int64)
    other_hour = other_df["hour_of_day"].to_numpy(dtype=np.int64)
...
def dcr_quantile(
    ...
    *,
    ...
    oth_day = other_df["day_of_week"].to_numpy(dtype=np.int64)
    oth_hour = other_df["hour_of_day"].to_numpy(dtype=np.int64)
...
def dcr_within_quantile(
    ...
    *,
    ...
    ref_day = df["day_of_week"].to_numpy(dtype=np.int64)
    ref_hour = df["hour_of_day"].to_numpy(dtype=np.int64)
