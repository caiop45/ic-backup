...
from tvae.utils.metrics import marginal_counts, plot_marginal_hist, save_metrics_json
from tvae.utils.serialization import build_model_from_checkpoint, load_checkpoint, load_mappings
...
def main() -> int:
    ...
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=Path(config.OUTPUT_BASE_DIR) / "comparison_plots",
    )
    ...
    save_root = Path(config.SAVE_DATA_DIR)
    runs = {
